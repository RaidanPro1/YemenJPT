...
const AICorePage: React.FC = () => {
  ...
  const cpuColorClass = cpuUsage > 90 ? 'text-red-500' : cpuUsage > 70 ? 'text-[#e1b000]' : 'text-blue-400';
  const cpuBgClass = cpuUsage > 90 ? 'bg-red-500' : cpuUsage > 70 ? 'bg-[#e1b000]' : 'bg-[#00338d]';
          ...
          <div className="p-10 bg-slate-950/80 border-t border-slate-800/50 backdrop-blur-2xl">
            <div className="flex gap-5 items-center bg-slate-900/90 rounded-[3rem] p-4 border border-slate-800 focus-within:border-[#00338d]/50 transition-all shadow-inner">
              <button 
                ...
                className={`p-5 rounded-2xl transition-all shadow-inner ${isRecording ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
              >
